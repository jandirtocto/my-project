...
export default function RegisterLastNameScreen({ navigation,route}:any){
    ...
    return(
       <View style={styles.view}>
        ...
        placeholder="to complete..." placeholderTextColor="#0F0F0F" onChangeText={setLastName} value={lastName}
        maxLength={20}/>
        <Text style={styles.textAlert} >{lastName.length===0?"campo requerido":null}{lastName.length===20?"minimo 10 caracteres":""}</Text>
        </View>
        <View  style={styles.viewButton}>
            <Button bolean={false} dirigir={()=>navigation.goBack()} />
            <Button bolean={true}  dirigir={()=>navigation.navigate("RegisterNumberScreen",{email,password,confirmPassword,firstName,lastName})}
